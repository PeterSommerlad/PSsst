...
// (c) copyright 2020-2023 Peter Sommerlad : Better Software
#include <cmath>
#include <type_traits>
#include <utility>
#include <stdexcept>
#ifdef __cpp_lib_destroying_delete
#include <new> // for std::destroying_delete_t
#endif
namespace pssst {
...
struct holder  {
  ...
  V value { };
#ifdef __cpp_impl_destroying_delete
  // provide some protection against implicit base class conversion
void operator delete( holder* ptr, std::destroying_delete_t ){
    // cannot check if TAG is subtype here directly, therefore indirect the static_cast
    destroyer_cast<TAG>(ptr)->~TAG();
    ::operator delete(static_cast<void*>(ptr));
  }
#endif
};
// apply multiple operator mix-ins and keep this an aggregate, but only if this is an indirect base as with strong
...
// however, there is the danger to actually have a derived to base pointer conversion
template <typename TAG, template <typename ...> class ...CRTP>
...
struct strong:detail_::holder<V,TAG>, detail_::mixinopsthisclassshouldneverbeusedforderivedtobaseconversion<TAG,OPS...> {
};
...
struct Cmp3way
{
  ...
  friend constexpr
  bool operator==(U const &l, Other const& r) noexcept
...
struct Out {
  ...
  operator<<(std::ostream &out, U const &r) {
    if constexpr (detail_::has_prefixc<U>){
      out << U::prefix;
    }
    auto const &[v]=r;
    out << v;
    if constexpr (detail_::has_suffixc<U>){
      out << U::suffix;
    }
    return out;
  }
