...
struct literGas: strong<double, literGas, Additive, Order, Out> {
  ...
  constexpr literGas(double d) : strong{d} {
  }
};

struct kmDriven: strong<double, kmDriven>, ScalarMultImpl<kmDriven, double>, Out<kmDriven> {
  constexpr static auto prefix = "driven ";
  constexpr static auto suffix = " km";
  constexpr kmDriven(double d) : strong{d} {
  }
};

struct literPer100km: strong<double, literPer100km, Eq, Out> {
  constexpr static auto suffix = " l/100km";
  constexpr literPer100km(double d) : strong{d} {
  }
};
struct kmpl: strong<double, kmpl, Eq, Out> {
  constexpr static auto suffix = " km/l";
  constexpr kmpl(double d) : strong{d} {
  }
};
...
struct liter: ops<liter, Additive, Order, Out> {
  ...
  constexpr explicit liter(double lit) :
      value{lit} {
  }
  ;
  double value{};
};
static_assert(sizeof(liter)==sizeof(double),"");
// ensure empty bases are squashed
static_assert(std::is_trivially_copyable<liter>::value,"");
// ensure efficient argument passing
