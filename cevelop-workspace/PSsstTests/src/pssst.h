...
template<typename U>
struct UPlus {
  friend constexpr U operator+(U const &r) noexcept(noexcept(+std::declval<underlying_value_type<U>>())) {
    auto const& [v] = r;
    return retval < U > (+v);
  }
};
...
struct BitOps {
  friend constexpr
  R& operator|=(R &l, R const &r) noexcept {
    auto& [vl] = l;
...
#define VT underlying_value_type<R>
  friend constexpr
  R& operator+=(R &l, R const &r) noexcept(noexcept(std::declval<VT&>() += std::declval<VT const &>())) {
    auto& [vl] = l;
    auto const& [vr] = r;
    vl += vr;
    return l;
  ...
  friend constexpr
  R operator+(R l, R const &r) noexcept(noexcept(l+=r)) {
    return l += r;
  }
#undef VT
};
...
struct Out {
  ...
  operator<<(std::ostream &out, U const &r) {
    if constexpr (detail__::has_prefixc<U>) {
      out << U::prefix;
    }
    auto const& [v] = r;
    out << v;
    if constexpr (detail__::has_suffixc<U>) {
      out << U::suffix;
    }
    return out;
  }
...
struct ScalarMultImpl: ScalarModulo<R, BASE, std::is_integral_v<BASE> && not std::is_same_v<std::decay_t<BASE>, bool>> {
  ...
  friend constexpr
  R& operator*=(R &l, BASE const &r) noexcept {
    auto& [vl] = l;
    vl *= r;
    return l;
  }
  ...
  friend constexpr
  R operator*(BASE const &l, R r) noexcept {
    return r *= l;
  }
  friend constexpr
  R& operator/=(R &l, BASE const &r) {
